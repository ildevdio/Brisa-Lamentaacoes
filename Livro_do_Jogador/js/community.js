/* =====================================================================
   ...
   FLUXO
   - Quando um jogador cria conteúdo novo (raça, classe, magia, técnica,
     ferramenta/item ou particularidade), o item NÃO é salvo direto no
     app local: vira uma SUGESTÃO (status "pending") numa fila
     compartilhada armazenada num Gist PÚBLICO do GitHub.
   - A aba "Sugestões" mostra a fila para o admin. Ele aprova (publica
     para todos) ou rejeita (remove), usando a senha de admin.
   - Itens aprovados ficam em "approved" e, ao abrir o app (ou ao dar
     refresh), são MESCLADOS nos catálogos de todos: raças, classes,
     magias, técnicas, itens e particularidades.
   - Personagens permanecem 100% LOCAIS (localStorage de cada jogador)
     e jamais passam por essa fila.

   CONFIGURAÇÃO NECESSÁRIA (dono) — continua igual:
     1) Crie um GIST PÚBLICO com um arquivo "catalog-contrib.json"
        contendo: { "pending": [], "approved": [] }
        Copie o ID do gist (após gist.github.com/<user>/) para GIST_ID.
     2) Gere um Personal Access Token (classic) com escopo "gist" e
        cole em GIST_TOKEN — ou deixe GIST_TOKEN_PLACEHOLDER e injete
        via secret do GitHub Actions (workflow pages.yml faz o sed).
   ===================================================================== */
...
/* Estado da fila em memória: { pending:[], approved:[] } */
...
/* =====================================================================
   LEITURA: baixa a fila do Gist (raw público, sem autenticação).
   Se falhar a rede, usa o último cache salvo no localStorage.
   ===================================================================== */
...
/* =====================================================================
   MESCLA itens aprovados nos catálogos (só adiciona se ainda não existe,
   por id). Atua sobre o DATA do app — nunca sobre personagens.
   ===================================================================== */
function commApply(){
  const data = DATA;
  if(!data) return;
...
/* =====================================================================
   ...
   Requer token (injetado no deploy ou em GIST_TOKEN). Devolve true/false.
   ===================================================================== */
...
async function communityAdd(type, data, password, playerName){
  ...
  const item = {
    id: commNewId(type), type: type, data: data,
    submittedBy: String(playerName||"").trim(),
    submittedAt: new Date().toISOString(),
    status: "pending"
  };
  await commWrite({ pending: commPending().concat([item]), approved: commApproved() });
  return item;
}

async function communityApprove(id, password){
  if(password !== COMMUNITY.PASSWORD_ADMIN) throw new Error("Senha de admin incorreta.");
  const p = commPending(), it = p.find(x=>x.id===id);
...
/* =====================================================================
   FORMULÁRIO DE SUGESTÃO (jogador): envia item para "pending".
   ===================================================================== */
function commAddForm(type){
  ...
  const titles = {
    raca:"Nova raça", classe:"Nova classe", magia:"Nova magia",
  ...
  openModal(titles[type], forms[type](), async (fd)=>{
    try{
      ...
      await communityAdd(type, base, fd.get("password"), fd.get("playerName"));
      closeModal();
...
/* =====================================================================
   CONVERSÃO: linha "Nome: Descrição" -> [{name,desc}] e caminhos "N|F|A".
   ===================================================================== */
...
/* =====================================================================
   MONTA o objeto do item a partir do FormData (mesmos campos do app).
   ===================================================================== */
function buildCommData(type, fd){
  switch(type){
    case "raca": return {
      id: commNewId(type), custom:true, name: fd.get("name").trim(),
      bonus:(fd.get("bonus")||"").trim(), appearance:(fd.get("appearance")||"").trim(),
      identity: fd.get("identity").trim(), traits: commFeatures(fd.get("traits"))
    };
    case "classe": return {
    ...
    case "ferramenta": return {
      id: commNewId(type), category: fd.get("category")||"Arma",
      tier: Number(fd.get("tier"))||1, name: fd.get("name").trim(),
      attr:(fd.get("attr")||"").trim(), range: fd.get("range")||"",
    ...
    case "particularidade": return {
      id: commNewId(type), name: fd.get("name").trim(),
      category:(fd.get("category")||"").trim()||"Geral",
...
/* =====================================================================
   ABA SUGESTÕES (admin): lista as pendentes com Aprovar/Rejeitar.
   ===================================================================== */
function renderSuggestions(){
  const wrap = document.getElementById("gridSugestoes");
  if(!wrap) return;
  const p = commPending();
  if(!p.length){
    wrap.innerHTML = `<div class="empty-state">Nenhuma sugestão aguardando aprovação. Itens criados pelos jogadores (botões "+ Nova ...") aparecem aqui para você aprovar.</div>`;
    return;
  }
    ...
    if(s.type === "raca"){
      ...
      inner = `
        ...
        <ul class="trait-list">
          ...
          ${(d.paths||[]).map(pv=>`<li><b>Caminho: ${escapeHtml(pv.name)}</b> (${escapeHtml(pv.focus||"")}) - ${escapeHtml(pv.areas||"")}</li>`).join("")}
        </ul>`;
...
function commPromptApprove(id){
  ...
  (async()=>{
    try{ await communityApprove(id, pw||""); await refreshCommunity(); showToast("Aprovado e publicado para todos."); }
...
function commPromptReject(id){
  ...
  (async()=>{
    try{ await communityReject(id, pw||""); await refreshCommunity(); showToast("Sugestão rejeitada."); }
    catch(e){ showToast(e.message); }
  })();
}
